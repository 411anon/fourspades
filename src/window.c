...
void window_title(char* suffix) {
	if(suffix) {
		char title[128];
		snprintf(title, sizeof(title) - 1, "fourspades %s - %s", BETTERSPADES_VERSION, suffix);
		glfwSetWindowTitle(hud_window->impl, title);
	} else {
		glfwSetWindowTitle(hud_window->impl, "fourspades " BETTERSPADES_VERSION);
	}
}
...
void window_init() {
	...
	*/
	...
	hud_window->impl
		= glfwCreateWindow(settings.window_width, settings.window_height, "fourspades " BETTERSPADES_VERSION,
...
void window_init() {
	...
	hud_window->impl
		= SDL_CreateWindow("fourspades " BETTERSPADES_VERSION, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
...
void window_title(char* suffix) {
	if(suffix) {
		...
		SDL_SetWindowTitle(hud_window->impl, "fourspades " BETTERSPADES_VERSION);
	}
}
